...
export type SessionUser = { id: string; companyId: string; name: string; email: string; role: string; companyName: string; verified: boolean; loginId: string; mustChangePassword: boolean }
...
export function getUserByEmail(emailOrLoginId: string) { return db.prepare('SELECT u.*, c.name company_name FROM users u JOIN companies c ON c.id=u.company_id WHERE lower(u.email)=lower(?) OR lower(u.employee_id)=lower(?)').get(emailOrLoginId, emailOrLoginId) as any }
...
export function toSessionUser(user: any): SessionUser { return { id:user.id, companyId:user.company_id, name:user.name, email:user.email, role:user.role, companyName:user.company_name, verified:Boolean(user.is_verified), loginId:user.employee_id, mustChangePassword:Boolean(user.must_change_password) } }
