...
export async function GET() {
  ...
  const leaves = db.prepare(`SELECT l.id, l.user_id userId, u.name, l.leave_type leaveType, l.start_date startDate, l.end_date endDate,
  ...
  const trend = user.role === 'employee'
    ? db.prepare(`SELECT user_id userId, date, COUNT(*) total, SUM(CASE WHEN check_in IS NOT NULL THEN 1 ELSE 0 END) present FROM attendance WHERE user_id=? AND date >= date('now','-6 day') GROUP BY user_id,date ORDER BY date`).all(user.id) as any[]
    : db.prepare(`SELECT date, COUNT(*) total, SUM(CASE WHEN check_in IS NOT NULL THEN 1 ELSE 0 END) present FROM attendance WHERE user_id IN (SELECT id FROM users WHERE company_id=?) AND date >= date('now','-6 day') GROUP BY date ORDER BY date`).all(user.companyId) as any[]
  ...
  const visibleEmployees = user.role === 'employee' ? employees.filter((employee) => employee.id === user.id) : employees
  const visibleLeaves = user.role === 'employee' ? leaves.filter((leave) => leave.userId === user.id) : leaves
  return NextResponse.json({ date, employees: visibleEmployees, leaves: visibleLeaves, trend: user.role === 'employee' ? trend.filter((day) => day.userId === user.id) : trend, metrics: { totalEmployees: user.role === 'employee' ? 1 : employees.length, presentToday: user.role === 'employee' ? Number(Boolean(ownAttendance?.checkIn)) : attendance, onLeaveToday: user.role === 'employee' ? (visibleLeaves.some((leave) => leave.status === 'approved' && leave.startDate <= date && leave.endDate >= date) ? 1 : 0) : onLeave.count, pendingApprovals: user.role === 'employee' ? visibleLeaves.filter((leave) => leave.status === 'pending').length : pending.count }, ownAttendance })
}
